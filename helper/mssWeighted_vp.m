function [ mss, tim ] = mssWeighted_vp( X, m, blksz, distance, fitfn,resfn,degenfn, win, epsi )
...
for i = 1:m
    if(i <= blksz)
        ...
        if (mod(i-1,blksz)==0)
            ...
            for itj=1:new_blk
                st=feval(fitfn,X(:,mss(itj,:)));
                st = reshape(st,3,1);
                H_cap(:,itj) = st;
                for itj1=1:n
                    R_cap(itj1,itj) = feval(resfn,X(:,itj1),st);
                end
               % R_cap(:,itj)=feval(resfn,X,st);
            end
            %             H_cap(:,new_blk) = feval(fitfn,X(:,mss(new_blk,:)));
            %             R_cap(:,new_blk) = feval(X, H_cap(:,new_blk), distFun );
            switch distance
        ...
        while(j<=cardmss)
            ...
            w = w.*w;                            % update weight
            %--------- ceck for degeneracy ---------%
            isdegen = feval(degenfn,X(:,mss(i,1:j)));
            if( ~isdegen )
                j = j+1;
            elseif(all(w==0))
